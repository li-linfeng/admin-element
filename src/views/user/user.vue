<template>
  ...
  >
     ...
     <el-table-column label="关联角色"  width="200" align="center">
        <template slot-scope="{row}">
          <el-tag type="success" v-for="item in row.roles"  :key="item.name" style="margin-right:3px;">
            {{ item.name }}
          </el-tag>
        </template>
    </el-table-column>
  ...
  <el-dialog :title="dialogAction == 'add' ? '增加' :'编辑'" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="tmp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item :label="dialogAction == 'add'?'密码':'新密码'" prop="password">
          <el-input v-model="tmp.password" type="password" />
        </el-form-item>
        <el-form-item label="角色" prop="roles">
          <el-select v-model="tmp.roles" multiple placeholder="请选择">
            <el-option
